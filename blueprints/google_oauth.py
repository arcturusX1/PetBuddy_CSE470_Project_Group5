...
@google_oauth_bp.route('/login_google')
def login():
    flow = Flow.from_client_secrets_file(CLIENT_SECRETS_FILE, 
                                         scopes=SCOPES
                                        )
    flow.redirect_uri = url_for('google_oauth_bp.oauth2callback', 
                                _external=True)
...
def oauth2callback():
    ...
    flow.redirect_uri = url_for('google_oauth_bp.oauth2callback', _external=True)
